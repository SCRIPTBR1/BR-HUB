...
-- 🌹 CORES DO HUB - VERMELHO, BRANCO, PRETO
local RED = Color3.fromRGB(220, 20, 20)
local DARK_RED = Color3.fromRGB(150, 10, 10)
local BRIGHT_RED = Color3.fromRGB(255, 50, 50)
local WHITE = Color3.fromRGB(255, 255, 255)
local BLACK = Color3.fromRGB(0, 0, 0)
local DARK_BLACK = Color3.fromRGB(8, 8, 8)
...
local function showDiscordInProgressBar()
	...
	progressPct.Text = "discord.gg/uS9c7sX9CA"
	...
	task.delay(4, function()
		if progressPct then
			...
			progressPct.TextColor3 = originalColor or WHITE
...
-- 🌹 CORES DO HUB
local BG = DARK_BLACK
local SIDEBAR_BG = BLACK
local CARD_BG = Color3.fromRGB(20, 5, 5)
local CARD_HOV = Color3.fromRGB(40, 8, 8)
local BORDER = RED
local BORDER2 = BRIGHT_RED
local DIM = GRAY
local DIM2 = Color3.fromRGB(120, 30, 30)
local KB_BG = Color3.fromRGB(25, 5, 5)
local INPUT_BG = Color3.fromRGB(30, 8, 8)

local W, H, SW = 490, 460, 90
local PW = 0
local CORNER = 12
local uiScaleValue = 70
...
verLbl.Size = UDim2.new(0, 150, 1, 0)
verLbl.Position = UDim2.new(0, 160, 0, 0)
verLbl.BackgroundTransparency = 1
verLbl.Text = "discord.gg/uS9c7sX9CA"
...
minBtn.BackgroundColor3 = Color3.fromRGB(30, 8, 8)
...
Instance.new("UIStroke", minBtn).Color = RED
minBtn.MouseEnter:Connect(function() TweenService:Create(minBtn, TweenInfo.new(0.1), {BackgroundColor3=Color3.fromRGB(60,15,15)}):Play() end)
minBtn.MouseLeave:Connect(function() TweenService:Create(minBtn, TweenInfo.new(0.1), {BackgroundColor3=Color3.fromRGB(30,8,8)}):Play() end)

local portrait = Instance.new("Frame", main)
portrait.Size = UDim2.new(0, PW, 1, -44)
portrait.Position = UDim2.new(0, 0, 0, 44)
portrait.BackgroundColor3 = BLACK
...
portImg.Image = "rbxassetid://101500203237017"
...
nameTag.Text = "🌹 BR DUELS"
nameTag.TextColor3 = RED
...
byTag.Text = "discord.gg/uS9c7sX9CA"
byTag.TextColor3 = WHITE
byTag.Font = Enum.Font.Gotham
byTag.TextSize = 9
byTag.TextXAlignment = Enum.TextXAlignment.Left
byTag.ZIndex = 6

do local _pd = Instance.new("Frame",main); _pd.Size=UDim2.new(0,1,1,-44); _pd.Position=UDim2.new(0,PW,0,44); _pd.BackgroundColor3=RED; _pd.BorderSizePixel=0; _pd.ZIndex=5 end
...
do local _sd=Instance.new("Frame",sidebar); _sd.Size=UDim2.new(0,1,1,0); _sd.Position=UDim2.new(1,-1,0,0); _sd.BackgroundColor3=RED; _sd.BorderSizePixel=0; _sd.ZIndex=6 end
...
mini.MouseEnter:Connect(function() TweenService:Create(mini,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(180,20,20)}):Play() end)
mini.MouseLeave:Connect(function() TweenService:Create(mini,TweenInfo.new(0.1),{BackgroundColor3=RED}):Play() end)
...
local ACTIVE_TAB_BG  = RED
local ACTIVE_TAB_TXT = WHITE
local IDLE_TAB_BG    = Color3.fromRGB(25, 5, 5)
...
for i, td in ipairs(tabDefs) do
	...
	btn.MouseEnter:Connect(function()
		if activeTabName ~= td.name then TweenService:Create(btn, TweenInfo.new(0.1), {BackgroundColor3=Color3.fromRGB(40,8,8)}):Play() end
	end)
...
local function baseCard(tabName, h2)
	...
	Instance.new("UIStroke", c).Color = RED
...
local function makePillToggle(parent, defOn, onToggle)
	...
	pbg.BackgroundColor3 = defOn and RED or Color3.fromRGB(20, 5, 5)
	pbg.BorderSizePixel = 0
	pbg.ZIndex = 8
	Instance.new("UICorner", pbg).CornerRadius = UDim.new(0, 10)
	local ps = Instance.new("UIStroke", pbg); ps.Color = defOn and RED or BORDER2; ps.Thickness = 1
	...
	dot.BackgroundColor3 = defOn and BLACK or DIM2
	...
	local function setV(on)
		isOn = on
		TweenService:Create(pbg, TweenInfo.new(0.18), {BackgroundColor3=on and RED or Color3.fromRGB(20, 5, 5)}):Play()
		TweenService:Create(ps,  TweenInfo.new(0.18), {Color=on and RED or BORDER2}):Play()
		TweenService:Create(dot, TweenInfo.new(0.18, Enum.EasingStyle.Back), {
			...
			BackgroundColor3 = on and BLACK or DIM2
		}):Play()
	end
...
local function makeKB(parent, kbEntry, onChange)
	...
	local bs = Instance.new("UIStroke", b); bs.Color = RED; bs.Thickness = 1
	...
	b.MouseButton1Click:Connect(function()
		...
		lc = UIS.InputBegan:Connect(function(inp)
			...
			if inp.KeyCode == Enum.KeyCode.Escape then
				...
				b.Text=pv; b.TextColor3=WHITE; TweenService:Create(bs,TweenInfo.new(0.1),{Color=RED}):Play(); return
			end
			if isGp then
			...
			b.TextColor3=WHITE
			li=false; _anyKeyListening=false; if lc then lc:Disconnect(); lc=nil end
			TweenService:Create(bs, TweenInfo.new(0.1), {Color=RED}):Play()
...
local function rowInput(tabName, label, sub, default, onChange)
	...
	local bs = Instance.new("UIStroke", box); bs.Color = RED; bs.Thickness = 1; bs.ZIndex = 12
...
local function rowActionBtn(tabName, label, onClick)
	...
	b.MouseButton1Click:Connect(function()
		TweenService:Create(b, TweenInfo.new(0.08), {BackgroundColor3=Color3.fromRGB(180,20,20)}):Play()
		task.delay(0.15, function() TweenService:Create(b, TweenInfo.new(0.1), {BackgroundColor3=RED}):Play() end)
		if onClick then pcall(onClick) end
	end)
	b.MouseEnter:Connect(function() TweenService:Create(b, TweenInfo.new(0.1), {BackgroundColor3=Color3.fromRGB(200,30,30)}):Play() end)
	b.MouseLeave:Connect(function() TweenService:Create(b, TweenInfo.new(0.1), {BackgroundColor3=RED}):Play() end)
	return b
end

local pbFrame = Instance.new("Frame", gui)
pbFrame.Size = UDim2.new(0, 320, 0, 50)
pbFrame.Position = UDim2.new(0.5, -160, 1, -70)
pbFrame.BackgroundColor3 = Color3.fromRGB(20, 5, 5)
pbFrame.BorderSizePixel = 0
pbFrame.Active = true
Instance.new("UICorner", pbFrame).CornerRadius = UDim.new(0, 10)
Instance.new("UIStroke", pbFrame).Color = RED
...
pbBg.Size = UDim2.new(1, -20, 0, 12)
pbBg.Position = UDim2.new(0, 10, 0, 28)
pbBg.BackgroundColor3 = Color3.fromRGB(30, 8, 8)
...
do
	...
	Instance.new("UIStroke", radValBtn).Color = RED
	local typing2 = false
...
local setInfJump = rowToggle("Mechanics", "Infinite Jump", nil, false, function(on) State.infJumpEnabled = on end)

local jumpModeContainer = Instance.new("Frame", pg("Mechanics"))
jumpModeContainer.Size = UDim2.new(1,0,0,34)
jumpModeContainer.BackgroundTransparency = 1
jumpModeContainer.BorderSizePixel = 0
jumpModeContainer.LayoutOrder = lo("Mechanics")
...
manuelBtn.BackgroundColor3 = State.infJumpMode == "manual" and RED or Color3.fromRGB(25, 5, 5)
...
Instance.new("UIStroke", manuelBtn).Color = RED
...
holdBtn.BackgroundColor3 = State.infJumpMode == "hold" and RED or Color3.fromRGB(25, 5, 5)
...
Instance.new("UIStroke", holdBtn).Color = RED

local function updateJumpModeUI()
    if State.infJumpMode == "manual" then
        manuelBtn.BackgroundColor3 = RED; manuelBtn.TextColor3 = WHITE
        holdBtn.BackgroundColor3 = Color3.fromRGB(25, 5, 5); holdBtn.TextColor3 = DIM
    else
        manuelBtn.BackgroundColor3 = Color3.fromRGB(25, 5, 5); manuelBtn.TextColor3 = DIM
        holdBtn.BackgroundColor3 = RED; holdBtn.TextColor3 = WHITE
    end
end
...
do
	...
	musicBtn.Activated:Connect(function()
		...
		pcall(saveConfig)
		TweenService:Create(musicBtn, TweenInfo.new(0.1), {BackgroundColor3=Color3.fromRGB(180,20,20)}):Play()
		task.delay(0.15, function()
			TweenService:Create(musicBtn, TweenInfo.new(0.1), {BackgroundColor3=RED}):Play()
		end)
		task.wait(0.5)
		isChangingMusic = false
	end)
	musicBtn.MouseEnter:Connect(function()
		TweenService:Create(musicBtn, TweenInfo.new(0.1), {BackgroundColor3=Color3.fromRGB(200,30,30)}):Play()
	end)
...
do
	...
	local Q_OFF = Color3.fromRGB(30, 8, 8)
	local Q_ON = RED
	local Q_TEXT_OFF = WHITE
	local Q_TEXT_ON = WHITE
	...
	local function createMobileButton(name, displayText, col, row, isToggle, onAction)
		...
		Instance.new("UICorner", btn).CornerRadius = UDim.new(0, 29)
		...
		local function flash()
			TweenService:Create(btn, TweenInfo.new(0.08), {BackgroundColor3=Color3.fromRGB(200,50,50), TextColor3=Q_TEXT_ON}):Play()
...
infJumpMode=State.infJumpMode, antiRagdoll=State.antiRagdollEnabled,
...
-- RAÍZES DAS FUNÇÕES DE DESYNC (versão simplificada)
do
local DS = {
...
local function setupMedusaCounter(char)
    ...
    for _,part in ipairs(char:GetDescendants()) do if part:IsA("BasePart") then 
        Conns.anchor[#Conns.anchor+1] = part:GetPropertyChangedSignal("Anchored"):Connect(function() 
            if part.Anchored and part.Transparency==1 then 
                local med=findMedusa()
                if med then pcall(function() med:Activate() end) end
            end 
        end)
    end end
    Conns.anchor[#Conns.anchor+1] = char.DescendantAdded:Connect(function(part) 
...
local function setupChar(char)
    ...
    if head then
        ...
        speedLbl.Text="0.0"; speedLbl.TextColor3=WHITE
        speedLbl.Font=Enum.Font.GothamBlack; speedLbl.TextScaled=true
        speedLbl.TextStrokeTransparency=0; speedLbl.TextStrokeColor3=BLACK
        local discordLbl=Instance.new("TextLabel",bb)
        discordLbl.Size=UDim2.new(1,0,0,28); discordLbl.Position=UDim2.new(0,0,0,26)
        discordLbl.BackgroundTransparency=1; discordLbl.Text="🌹 BR DUELS"
        discordLbl.TextColor3=RED; discordLbl.Font=Enum.Font.GothamBold
        discordLbl.TextScaled=true; discordLbl.TextStrokeTransparency=0.1
        discordLbl.TextStrokeColor3=BLACK
    end
...
-- BILLBOARD PARA OUTROS JOGADORES
...
RunService.RenderStepped:Connect(function()
    ...
    if not State.autoBatToggled and not State.autoLeftEnabled and not State.autoRightEnabled then
        ...
        elseif State.antiRagdollEnabled and State.lastMoveDir.Magnitude>0 then
            local anyHeld=false; for key in pairs({[Enum.KeyCode.W]=true,[Enum.KeyCode.A]=true,[Enum.KeyCode.S]=true,[Enum.KeyCode.D]=true,
                [Enum.KeyCode.Up]=true,[Enum.KeyCode.Left]=true,[Enum.KeyCode.Down]=true,[Enum.KeyCode.Right]=true}) do if UIS:IsKeyDown(key) then anyHeld=true; break end end
...
-- COLLISION OFF FOR OTHER PLAYERS
...
loadConfig = function()
    ...
    if cfg.infJumpMode == "manual" or cfg.infJumpMode == "hold" then
        ...
        if updateJumpModeUI then updateJumpModeUI() end
    end
...
local function playIntroAnimation()
	...
	introFrame.BackgroundColor3 = BLACK
	...
	hubLabel.Text = "BR DUELS"
	hubLabel.TextColor3 = RED
	hubLabel.TextTransparency = 0
	hubLabel.TextSize = 88
	hubLabel.Font = Enum.Font.GothamBold
	hubLabel.TextStrokeColor3 = WHITE
...
-- INICIALIZAR TAB PRINCIPAL
switchTab("Speed")

print("🌹 BR DUELS Carregado!")
end)()

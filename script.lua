...
local function addShimmerToLabel(lbl,color1,color2)
    ...
    gr.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,color1 or Color3.fromRGB(200,0,0)),ColorSequenceKeypoint.new(0.5,color2 or Color3.fromRGB(255,255,255)),ColorSequenceKeypoint.new(1,color1 or Color3.fromRGB(200,0,0))})
...
local function createRagdollBillboard(duration,labelText,color)
    ...
    local RED  = color or Color3.fromRGB(200,0,0)
    ...
    stroke.Color=RED;stroke.Thickness=3;stroke.Transparency=0.05
    task.spawn(function()
        local t=0
        while stroke and stroke.Parent do
            t=t+0.05
            stroke.Transparency=0.02+math.abs(math.sin(t*2.5))*0.25
            stroke.Color=Color3.fromRGB(200,math.clamp(math.floor(RED.G*255+math.abs(math.sin(t*1.5))*35),0,255),math.clamp(math.floor(RED.B*255+math.abs(math.sin(t*1.5))*25),0,255))
            task.wait(0.04)
        end
    end)
    ...
    divider.BackgroundColor3=RED;divider.BackgroundTransparency=0.5;divider.BorderSizePixel=0;divider.ZIndex=31
    ...
    timerLbl.TextColor3=RED;timerLbl.Font=Enum.Font.GothamBlack;timerLbl.TextSize=24
    timerLbl.TextXAlignment=Enum.TextXAlignment.Center;timerLbl.ZIndex=32
    local shimmer=addShimmerToLabel(timerLbl,RED,WHITE)
...
local function onHumanoidStateChanged(old,new)
    ...
    if isRag and not hum.PlatformStand and not activeBatBillboard then
        activeBatBillboard=createRagdollBillboard(2.6,"RAGDOLL",Color3.fromRGB(200,0,0))
...
local function onMedusaStateChanged()
    ...
    if hum and hum.PlatformStand and not activeMedusaBillboard then
        activeMedusaBillboard=createRagdollBillboard(4.5,"STONE",Color3.fromRGB(200,0,0))
...
local function setupSpeedIndicator(char)
    ...
    discordLabel.TextColor3=Color3.fromRGB(200,50,50);discordLabel.Font=Enum.Font.GothamBold;discordLabel.TextScaled=true;discordLabel.TextStrokeTransparency=0
    ...
    local gr1=addShimmerToLabel(speedLabel,Color3.fromRGB(200,0,0),Color3.fromRGB(255,255,255))
    local gr2=addShimmerToLabel(discordLabel,Color3.fromRGB(200,50,50),Color3.fromRGB(255,255,255))
    ...
    aimbotConn=RunService.RenderStepped:Connect(function()
        ...
        pcall(function()
            if not _predBall then
                ...
                _predBall.Color=Color3.fromRGB(200,0,0)
...
refreshSpeedModeLabel=function()
    ...
    if laggerModePillRef and laggerModePillRef.pill and laggerModePillRef.dot then
        ...
        local RED2=Color3.fromRGB(180,0,0);local OFF=Color3.fromRGB(30,10,10);local WHITE=Color3.fromRGB(255,255,255);local GRAY=Color3.fromRGB(120,80,80)
        TweenService:Create(pill,TweenInfo.new(0.16,Enum.EasingStyle.Quad),{BackgroundColor3=on and RED2 or OFF}):Play()
...
local function createStealBar()
    ...
    local WHITE=Color3.fromRGB(255,255,255)
    local BARBG=Color3.fromRGB(15,8,8)
    ...
    local sbStroke=Instance.new("UIStroke",stealBarFrame);sbStroke.Color=RED;sbStroke.Thickness=2;sbStroke.Transparency=0.1
    task.spawn(function()
        local t=0
        while sbStroke and sbStroke.Parent do
            t=t+0.05;sbStroke.Transparency=0.05+math.abs(math.sin(t*2))*0.35
            sbStroke.Color=Color3.fromRGB(200,math.floor(0+math.abs(math.sin(t*1.5))*30),math.floor(0+math.abs(math.sin(t*1.5))*20))
            task.wait(0.04)
        end
    end)
    local fillLine=Instance.new("Frame",stealBarFrame);fillLine.Size=UDim2.new(0,0,1,0)
    fillLine.BackgroundColor3=RED;fillLine.BorderSizePixel=0;fillLine.ZIndex=21
    Instance.new("UICorner",fillLine).CornerRadius=UDim.new(1,0)
    local fillGrad=Instance.new("UIGradient",fillLine)
    fillGrad.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(150,0,0)),ColorSequenceKeypoint.new(0.5,Color3.fromRGB(200,50,50)),ColorSequenceKeypoint.new(1,Color3.fromRGB(150,0,0))})
    ...
    pctLbl.TextColor3=RED;pctLbl.Font=Enum.Font.GothamBlack;pctLbl.TextSize=12
    ...
    div1.BackgroundColor3=RED;div1.BackgroundTransparency=0.6;div1.BorderSizePixel=0;div1.ZIndex=25
...
local function buildMobileButtons()
    ...
    mobGuiRef=mobGui
    local RED_ON      = Color3.fromRGB(200,0,0)
    local BTN_BG_OFF   = Color3.fromRGB(30,10,10)
    local PANEL_BG     = Color3.fromRGB(20,8,8)
    local BORDER_OFF   = Color3.fromRGB(100,30,30)
    local BORDER_ON    = Color3.fromRGB(200,0,0)
    local TEXT_OFF     = Color3.fromRGB(200,150,150)
    ...
    panelStroke.Color=Color3.fromRGB(80,20,20);panelStroke.Thickness=2;panelStroke.Transparency=0.3
    ...
    for i,def in ipairs(buttons) do
        ...
        local function setOn(v)
            ...
            if v then
                ...
                TweenService:Create(frame,TweenInfo.new(0.12,Enum.EasingStyle.Quad),{BackgroundColor3=RED_ON}):Play()
        ...
        local function flash()
            TweenService:Create(frame,TweenInfo.new(0.06),{BackgroundColor3=Color3.fromRGB(200,80,80)}):Play()
...
local function buildGui()
    ...
    local BG         = Color3.fromRGB(10, 5, 5)
    local BG2        = Color3.fromRGB(20, 8, 8)
    local ROW_BG     = Color3.fromRGB(18, 6, 6)
    local ROW_BORDER = Color3.fromRGB(60, 20, 20)
    local WHITE      = Color3.fromRGB(255, 255, 255)
    local GRAY       = Color3.fromRGB(120, 80, 80)
    local INP        = Color3.fromRGB(8, 3, 3)
    local RED        = Color3.fromRGB(200, 0, 0)
    local RED2       = Color3.fromRGB(180, 0, 0)
    local RED3       = Color3.fromRGB(80, 10, 10)
    local OFF        = Color3.fromRGB(30, 10, 10)
    local TAB_ACTIVE = Color3.fromRGB(200, 0, 0)
    local TAB_INACT  = Color3.fromRGB(120, 80, 80)
    local SECT_LBL   = Color3.fromRGB(180, 100, 100)
    for _,n in ipairs({"MoveeDuels"}) do        local old=game:GetService("CoreGui"):FindFirstChild(n);if old then old:Destroy() end
    ...
    local mfStroke=Instance.new("UIStroke",mainFrame);mfStroke.Color=RED;mfStroke.Thickness=1.5;mfStroke.Transparency=0.3
    task.spawn(function()
        local t=0
        while mfStroke and mfStroke.Parent do
            t=t+0.05
            mfStroke.Transparency=0.4+math.sin(t*3)*0.25
            mfStroke.Color=Color3.fromRGB(200,math.floor(0+math.sin(t*2)*20),math.floor(0+math.sin(t*2)*10))
            task.wait(0.04)
        end
    end)
    ...
    hDivider.BackgroundColor3=RED;hDivider.BackgroundTransparency=0.6;hDivider.BorderSizePixel=0;hDivider.ZIndex=7
    dragHandle(header)
    local titleLbl=Instance.new("TextLabel",header)
    titleLbl.Size=UDim2.new(1,-80,1,0);titleLbl.Position=UDim2.new(0,12,0,0)
    titleLbl.BackgroundTransparency=1;titleLbl.Text="🌹BR DUELS"
    ...
    discSmall.TextColor3=RED;discSmall.Font=Enum.Font.GothamBold
    ...
    closeBtn.BackgroundColor3=RED3;closeBtn.BorderSizePixel=0
    ...
    miniBtn.Text="🌹BR DUELS";miniBtn.TextColor3=RED
    ...
    for _,name in ipairs(TABS) do
        ...
        sf.ScrollBarThickness=2;sf.ScrollBarImageColor3=RED3
    ...
    for _,name in ipairs(TABS) do
        ...
        underline.BackgroundColor3=RED;underline.BorderSizePixel=0
    ...
    local function animPill(pill, dot, on)
        TweenService:Create(pill,TweenInfo.new(0.15,Enum.EasingStyle.Quad),{BackgroundColor3=on and RED2 or OFF}):Play()
    ...
    local function mkBoxRow(parent, txt, default, cb)
        ...
        tb.Text=tostring(default);tb.TextColor3=RED
    ...
    local function mkKBButton(row, kbEntry, cb)
        ...
        btn.Text=getLabel();btn.TextColor3=RED
        ...
        btn.Activated:Connect(function()
            if li then li=false;_anyKeyListening=false;if lc then lc:Disconnect();lc=nil end;btn.Text=pv;btn.TextColor3=RED;return end
            ...
            lc=UIS.InputBegan:Connect(function(inp)
                ...
                if inp.KeyCode==Enum.KeyCode.Escape then li=false;_anyKeyListening=false;if lc then lc:Disconnect();lc=nil end;btn.Text=pv;btn.TextColor3=RED;return end
    ...
    local function mkActionRow(parent, txt, onActivate)
        ...
        btn.BackgroundColor3=RED3;btn.BorderSizePixel=0
        btn.Text="▶";btn.TextColor3=WHITE;btn.Font=Enum.Font.GothamBlack;btn.TextSize=11
        btn.AutoButtonColor=false;btn.ZIndex=9
        Instance.new("UICorner",btn).CornerRadius=UDim.new(0,5)
        btn.Activated:Connect(function()
            TweenService:Create(btn,TweenInfo.new(0.06),{BackgroundColor3=Color3.fromRGB(120,20,20)}):Play()
            task.delay(0.12,function() TweenService:Create(btn,TweenInfo.new(0.1),{BackgroundColor3=RED3}):Play() end)
    ...
    do
        ...
        manualBtn.BackgroundColor3=(infJumpMode=="manual") and RED2 or Color3.fromRGB(40,10,10)
        ...
        holdBtn.BackgroundColor3=(infJumpMode=="hold") and RED2 or Color3.fromRGB(40,10,10)
        ...
        manualBtn.Activated:Connect(function() infJumpMode="manual";manualBtn.BackgroundColor3=RED2;holdBtn.BackgroundColor3=Color3.fromRGB(40,10,10);stopHoldInfJump();saveConfig() end)
        holdBtn.Activated:Connect(function() infJumpMode="hold";holdBtn.BackgroundColor3=RED2;manualBtn.BackgroundColor3=Color3.fromRGB(40,10,10);if infJumpEnabled then startHoldInfJump() end;saveConfig() end)
    end
    pg=tabPages["Steal"]
    ...
    do
        ...
        skyVal.TextColor3=RED;skyVal.Font=Enum.Font.GothamBold;skyVal.TextSize=9
        ...
        cycleBtn.BackgroundColor3=RED3;cycleBtn.BorderSizePixel=0
    ...
    do
        ...
        fovBtn.Text=tostring(fovValue);fovBtn.TextColor3=RED
    ...
    do
        ...
        resetBtn.BackgroundColor3=Color3.fromRGB(100,10,10);resetBtn.BorderSizePixel=0
    ...
    do
        ...
        sizeBox.Text=tostring(mobileButtonsSize);sizeBox.TextColor3=RED
    ...
    do
        ...
        resetPosBtn.BackgroundColor3=RED3;resetPosBtn.BorderSizePixel=0
